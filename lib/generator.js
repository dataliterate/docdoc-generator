...
function generate(pathToDocumentation) {

  docroot = path.normalize(pathToDocumentation);
  docdocroot = path.normalize(docroot + '_docdoc/');
  siteroot  = path.normalize(docroot + '_site/');

  PluginManager.init(docroot + '_docdoc', docroot, siteroot);
...
function renderPage(def, nav, depth) {
  ...
  tmpl.ejs(docdocroot  + 'theme/layout.ejs', page, function(err, html) {
    if(err) throw err;
