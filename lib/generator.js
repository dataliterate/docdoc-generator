...
function generate(_docdoc) {
  ...
  Step(
    function() {
      return loadConfig(null, docroot);
    },
    function(err) {
      if (err) { return console.error(err); }
      return prepareLayout(err, docroot , this);
    },
    function(err) {
      if (err) { return console.error(err); }
