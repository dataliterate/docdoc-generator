...
var docroot = path.normalize('./example-documentation/');
var siteroot  = path.normalize('./example-documentation/_site/');

PluginManager.init(docroot, siteroot);
...
function prepareLayout(err, docroot , cb) {
  ...
  ncp(docroot  + '_theme/_assets', docroot  + '/_site/_assets', function (err) {
    if (err) { return console.error(err); }
    console.log('done!');
    cb();
  });
...
function renderPacket(def) {
  var content = ghm.parse(def.content);
  var assets = '';
...
function renderPage(def, nav, depth) {
  ...
  tmpl.ejs(docroot  + '_theme/layout.ejs', page, function(err, html) {
    if(err) throw err;
...
function doGenerate(err, tree) {
  ...
  if (libutil.exists('_site', true)) rmrf.sync('_site');
  ...
  if(filepath.indexOf(docroot ) == 0) {
    filepath = filepath.substr(docroot .length);
  }
  
  var out = docroot  + '_site/' + filepath.substring(0, filepath.length - 1) + '.html';
  ensureDirectories(out);
