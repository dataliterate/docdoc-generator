...
PluginContext.prototype = {
  ...
  init: function() {
    this.assets = '_assets/plugin/' + this.plugin.name + '/';
  },

  generate: function(media, config, context) {
    var html = '<div class="plugin-' + this.plugin.name + '">';
    html += this.plugin.generate(media, config, context);
    html += '</div>';
    return html;
  },
  ...
  return function() {
    ...
    ncp(plugin.theme, destination, {
      ...
      prepareLayoutFunctions.push(function() {
        console.log('prepare layout for plugin ' + plugin.name + ' to _site ');
        ...
        ncp(destination + '/_assets', siteDestination, {
          ...
          if(err) {
            console.log('prepare layout ' + plugin.name + ' to _site .. error');
            pd.reject(err);
            console.log(err);
            return;
          }
